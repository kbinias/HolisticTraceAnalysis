...
def _filter_gpu_kernels_with_cuda_sync(
    ...
    return ((df["stream"] >= 0) & (df["correlation"] >= 0)) | ((df["syclqueue"] >= 0) & (df["correlation"] >= 0)) | df["name"].isin(
...
class GPUKernelFilter(Filter):
    ...
    def __call__(
        ...
        if "stream" not in df.columns and "syclqueue" not in df.columns:
            logger.warning("DataFrame is missing the required 'stream' or 'syclqueue' column.")
            return df
